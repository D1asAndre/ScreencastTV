...
if ($update) {
    ...
    $descricao = filter_input(INPUT_POST, 'descricao', FILTER_SANITIZE_STRING);
        ...
        if ($stmt->execute()) {
            $result .= '<div class="alert-success">Playlist atualizado com sucesso!</div>';
        } else {
            $result .= '<div class="alert-danger">Erro ao atualizar Playlist.</div>';
        }
} 
...
<form action="?m=<?= $module ?>&a=update&id=<?= $id ?>" role="form" method="post">
    ...
    <div class="form-group">
        ...
        <input name="nome" id="nome" type="text" value="<?= $reg['nome'] ?>" class="form-control">
    </div>
